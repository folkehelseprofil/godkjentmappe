...
# F:\Prosjekter\Kommunehelsa\PRODUKSJON\PRODUKTER\KUBER\OVP_KOMM\2018\GODKJENT\
...
dir.create(paste(globs$path,"/",globs$ovpDir_K,"/",KHaar,"/GODKJENT",sep=""))
newdir<-paste(globs$path,"/",globs$ovpDir_K,"/",KHaar,"/GODKJENT/",batchdate,sep="")
dir.create(newdir)

for (i in 1:nrow(FriskvikBatch)){
  orgfil<-paste(globs$path,"/",globs$ovpDir_K,"/",KHaar,"/CSV/",FriskvikBatch[i,1],".csv",sep="")
