...
source('F:/Prosjekter/Kommunehelsa/PRODUKSJON/BIN/KHfunctions_20200103.r')
if (grepl("\\S",args[1])){
  batchdate<-args[1]
} else {
...
dir.create(paste(globs$path,"/",globs$FriskVDir_F,"/",KHaar,"/GODKJENT",sep=""))
newdir<-paste(globs$path,"/",globs$FriskVDir_F,"/",KHaar,"/GODKJENT/",batchdate,sep="")
dir.create(newdir)

for (i in 1:nrow(FriskvikBatch)){
  orgfil<-paste(globs$path,"/",globs$FriskVDir_F,"/",KHaar,"/CSV/",FriskvikBatch[i,1],".csv",sep="")
